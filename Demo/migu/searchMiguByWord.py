        ...
        try:
            ...
            for search_word in temp_list:
                ...
                if fail_flag:
                    continue
                song_list = ''
                if response.find('data') != -1 and response.find('result') != -1:
                    song_list = json.loads(response)['data']['result']
                if len(song_list) > 0:
                    json.loads(response)['data']['result'][0]['name']
                    json.loads(response)['data']['result'][0]['singers']
                    json.loads(response)['data']['result'][0]['albums'][0]['name']
                    singer = ''
                    first_song = song_list[0]
                    result__sheet.write(i, 3, first_song['name'])
                    for single_singer in first_song['singers']:
                        singer = singer + single_singer['name'] + '&'
                    if singer.find('&'):
                        singer = singer[0:len(singer) - 1]
                    result__sheet.write(i, 4, singer)
                    result__sheet.write(i, 5, first_song['albums'][0]['name'])
                    # print(search_word)
...
def getMiGuDictByWord(search_word):
    result_list = []
    for request_time in range(0, 3):
        response = str(getHtml(search_word))
        if response != '':
    ...
    if len(song_list) > 0:
        ...
        for each_song in song_list:
            ...
            each_song_info = {}
            singer = ''
            for single_singer in each_song['singers']:
                singer = singer + single_singer['name'] + '&'
            if singer.find('&'):
                singer = singer[0:len(singer) - 1]
...
def getHtml(search_word):
    ...
    try:
        ...
        url = 'http://223.111.8.196:8886/slavemusicsearch/api/json/searchsong?text=' + parse.quote(str(search_word)) + \
              '&searchType=2&sort=1&issemantic=1&isCorrect=1&isCopyright=0&expire=000000&appId=test' \
              '&timestamp=1531061686&sid=1&sessionId=1&token=13fab9151bc2aa7787823db643398f65&pageSize=20&pageIndex=1'
