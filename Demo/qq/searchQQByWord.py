...
def getQQDictByWord(search_word):
    result_list = []
    for request_time in range(0, 3):
        ...
        if request_time == 2:
            ...
            i += 1
        continue
    json_list = response[response.index('(') + 1: len(response) - 1]
    song_list = json.loads(json_list)['data']['song']['list']
    if len(song_list) > 0:
        i = 0
        for each_song in song_list:
            ...
            each_song_info = {}
            singer = ''
            for single_singer in each_song['singer']:
                singer = singer + single_singer['name'] + '&'
            if singer.find('&'):
                singer = singer[0:len(singer) - 1]
...
def getHtml(search_word):
    ...
    try:
        ...
        url = "https://c.y.qq.com/soso/fcgi-bin/client_search_cp?ct=24&qqmusic_ver=1298&new_json=1" \
              "&remoteplace=txt.yqq.center&searchid=37327371824845865&t=0&aggr=1&cr=1&catZhida=1&lossless=0" \
              "&flag_qc=0&p=1&n=20&w=" + parse.quote(str(search_word)) + \
              "&g_tk=875285680&jsonpCallback=MusicJsonCallback20833915020969407" \
              "&loginUin=1260248494&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq" \
              "&needNewCode=0"
...
# startThread('E:/hot_word_1.xls', 'E:/hot_word_1_result.xls')
# end_time = time.time()
